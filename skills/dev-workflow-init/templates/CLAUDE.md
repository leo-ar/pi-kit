...
Every core component should be backed by three logical test blocks/files (using `fast-check` for JS/TS or `Eris` for PHP):

- **Examples** (`*.examples.test.ts`, `*ExampleTest.php`) — Illustrative, 1:1 input-to-output behavioral maps.
- **Errors** (`*.constraints.test.ts`, `*ConstraintTest.php`) — Validating strict constraint boundaries, type violations, and matching exceptions.
- **Properties** (`*.properties.test.ts`, `*PropertyTest.php`) Generative suites validating invariants that must hold true across all randomized data.
