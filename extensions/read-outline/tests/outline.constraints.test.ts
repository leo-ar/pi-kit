...
describe("generateOutline — constraints / edge cases", () => {
  it("returns empty array for empty file", () => {
    const entries = generateOutline([], "file.ts");
  ...
  it("returns empty array for unsupported language dispatched to generic with no matches", () => {
    const lines = [
      "just some random text",
      "with no recognizable patterns",
      "nothing here",
    ];
    const entries = generateOutline(lines, "file.swift");
    assert.deepEqual(entries, []);
  });

  it("handles single-line type alias", () => {
  ...
  it("does not crash on very short Python file", () => {
    const lines = ["x = 1"];
  ...
  it("handles Go file with only const", () => {
