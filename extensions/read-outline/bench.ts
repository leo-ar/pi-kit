...
interface Stats {
  ...
  byLanguage: Record<
    string,
    { count: number; originalChars: number; outlineChars: number }
  >;
...
async function main() {
  ...
  for (const dir of sessionDirs) {
    ...
    for (const file of jsonlFiles) {
      ...
      const resultEntries = parsed.filter((e) => e.type === "toolResult") as {
        type: "toolResult";
        text: string;
      }[];
      const callEntries = parsed.filter((e) => e.type === "readCall") as {
        type: "readCall";
        id: string;
        args: Record<string, unknown>;
      }[];
      ...
      let pendingRead: { id: string; args: Record<string, unknown> } | null =
        null;
      for (const entry of parsed) {
        if (entry.type === "readCall") {
          ...
          if (verbose) {
            ...
            console.log(
              `  ${filePath} (${fileLines.length} lines) → ${pct}% reduction`,
            );
          }
  ...
  console.log(
    `Sessions scanned:       ${stats.totalSessions} (${stats.totalSessionFiles} files)`,
  );
  ...
  console.log(
    `Above ${LINE_THRESHOLD}-line threshold: ${stats.aboveThreshold}`,
  );
  ...
  if (stats.outlineProduced > 0) {
    ...
    const savingsPct = (
      (savingsChars / stats.totalOriginalChars) *
      100
    ).toFixed(1);
    ...
    for (const [ext, data] of sorted) {
      const pct = ((1 - data.outlineChars / data.originalChars) * 100).toFixed(
        0,
      );
      console.log(
        `  ${ext.padEnd(6)} ${String(data.count).padStart(4)} files  ${pct}% avg reduction`,
      );
    }
    console.log("");
  ...
  if (stats.totalReadCalls > 0) {
    ...
    console.log(
      `  → ${stats.fullFileReads} full-file (${fmtPct(stats.fullFileReads, stats.totalReadCalls)})`,
    );
    console.log(
      `  → ${stats.supportedFileReads} supported ext (${fmtPct(stats.supportedFileReads, stats.totalReadCalls)})`,
    );
    console.log(
      `  → ${stats.aboveThreshold} above threshold (${fmtPct(stats.aboveThreshold, stats.totalReadCalls)})`,
    );
    console.log(
      `  → ${stats.outlineProduced} outlines produced (${fmtPct(stats.outlineProduced, stats.totalReadCalls)})`,
    );
  }
}
