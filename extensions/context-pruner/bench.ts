...
function benchSession(filePath: string, sampleEvery = 3): SessionResult {
  ...
  for (let t = 0; t < userTurnIndices.length; t++) {
    ...
    const contextEnd =
      t + 1 < userTurnIndices.length ? userTurnIndices[t + 1] : messages.length;
...
function main() {
  ...
  const aggregates: Record<
    string,
    { totalSaved: number; totalChars: number; avgPct: number }
  > = {};

  for (const name of configNames) {
    const totalSaved = allSlices.reduce(
      (s, sl) => s + sl.configs[name].charsSaved,
      0,
    );
    const totalChars = allSlices.reduce((s, sl) => s + sl.totalChars, 0);
    const avgPct =
      allSlices.length > 0
        ? allSlices.reduce((s, sl) => s + sl.configs[name].pctSaved, 0) /
          allSlices.length
        : 0;
  ...
  for (const name of configNames) {
    ...
    console.log(
      `  │  ${name.padEnd(6)}  │  ${(totalKB + " KB").padEnd(10)}  │  ${(avgKB + " KB/call").padEnd(10)}  │  ${(pct + "%").padStart(6)}  │`,
    );
  }
  console.log("  └──────────┴──────────────┴──────────────┴──────────┘\n");
  ...
  if (verbose) {
    ...
    for (const r of results) {
      ...
      console.log(
        `  ${r.file} (${r.userTurns} turns, ${r.totalMessages} msgs)`,
      );
      for (const name of configNames) {
        const cfg = lastSlice.configs[name];
        console.log(
          `    ${name}: ${cfg.pruned} pruned, ${(cfg.charsSaved / 1024).toFixed(1)}KB saved (${cfg.pctSaved.toFixed(1)}%)`,
        );
      }
      console.log();
    }
    ...
    console.log(
      "─── Savings Curve (avg % saved by turn index) ──────────────\n",
    );
    const turnBuckets = new Map<
      number,
      { count: number; pctK5: number; pctK10: number }
    >();
    for (const slice of allSlices) {
      const bucket = Math.floor(slice.turnIndex / 5) * 5;
      const existing = turnBuckets.get(bucket) || {
        count: 0,
        pctK5: 0,
        pctK10: 0,
      };
    ...
    const sortedBuckets = [...turnBuckets.entries()].sort(
      (a, b) => a[0] - b[0],
    );
    for (const [bucket, data] of sortedBuckets) {
      ...
      console.log(
        `  Turn ${String(bucket).padStart(3)}-${String(bucket + 4).padStart(3)}: K5 ${avgK5.padStart(5)}% ${bar5}  K10 ${avgK10.padStart(5)}% ${bar10}`,
      );
    }
    console.log();
  }
  ...
  console.log(
    `  Difference: K5 saves ${(k5Pct - k10Pct).toFixed(1)}% more than K10\n`,
  );

  if (k5Pct > 10) {
    console.log(
      "  ✅ Significant savings. Context pruner is pulling its weight.",
    );
  } else if (k5Pct > 5) {
