...
- `ToolResultMessage` — `role: "toolResult"`, `toolName`, `toolCallId`,
  `content: TextContent[]`, `isError`
- `AssistantMessage` — tool calls in `content[]` as
...
5. For each old `ToolResultMessage` above min size, classifies and replaces
   content
